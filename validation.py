...
# performs bootstrapping subset of data, with specific hyperparameters
def bootstrap(B, X, y, C_dict, Classifier):
    ...
    # return mean error across all bootstrapped samples
    err = np.mean(bs_err)
    return err


# implements k-fold cross validation with nested bootstrapping for hyperparameter tuning
def kfold(k, X, y, Classifier, arg_dict):
    ...
    for i in range(k):
        # compute training / test set indexes
        bot = (n * i) / k
        top = ((n * (i + 1)) / k)
